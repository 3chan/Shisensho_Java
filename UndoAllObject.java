class UndoAllObject extends IconObject {
    ...
    protected void OnClicked() {
        ...
        for (int i = 0; i < 144; i++) {
            if (((Game_Scene) getLayer().getScene()).obj_pieces[i] == null)
                continue;
        ...
        for (int i = 0; i < 144; i++) {
            ...
            System.out.println("hisAll[" + i + "].getPieceTexture() = " + hisAll[i].getPieceTexture() + ", obj_pieces["
                    + i + "].getPieceTexture() = "
                    + ((Game_Scene) getLayer().getScene()).obj_pieces[i].getPieceTexture());
        }
